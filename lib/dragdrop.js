...
function createWordElement(word, isInDropzone) {
  ...
  if (isInDropzone) {
    ...
    el.addEventListener("dragend", (e) => {
      ...
      if (
        ...
        const targetX = endRect.left + 10 + wordChoices.children.length * 80;
        ...
        setTimeout(() => {
          clone.remove();
          const newEl = createWordElement(word, false);
          wordChoices.appendChild(newEl);
  ...
  el.addEventListener("click", () => {
    ...
    setTimeout(() => {
      clone.remove();
      el.remove();
      const newEl = createWordElement(word, !isInDropzone);
      target.appendChild(newEl);

      if (dropzone.children.length === 0) disableCheckButton();
      else enableCheckButton();
    }, 300);
  });
