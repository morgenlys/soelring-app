...
function createWordElement(word, isInDropzone, index = null) {
  ...
  el.addEventListener("click", () => {
    ...
    let targetRect;
    if (isInDropzone) {
      const slot = wordChoices.querySelector(`.word-slot[data-index="${el.dataset.originIndex}"]`);
      if (slot) {
        slot.innerHTML = "";
        slot.appendChild(createPlaceholder(rect));
      ...
      tempEl.className = "word";
      tempEl.style.visibility = "hidden";
      tempEl.textContent = word;
      dropzone.appendChild(tempEl);
      targetRect = tempEl.getBoundingClientRect();
      dropzone.removeChild(tempEl);
    }
