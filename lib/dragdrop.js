...
function createWordElement(word, isInDropzone, index = null) {
  ...
  // ✅ Vollständig korrigierter Click-Handler
  el.addEventListener("click", () => {
    ...
    // Warte auf nächsten Frame → sicheres Layout
    requestAnimationFrame(() => {
      el.remove();

      const targetRect = target.getBoundingClientRect();
      const targetX = targetRect.left + 10 + target.children.length * (width + 10) + scrollX;
