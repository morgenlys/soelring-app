...
function createPlaceholder(rect) {
  ...
  placeholder.style.width = `${Math.max(rect.width, 50)}px`;
  placeholder.style.height = `${Math.max(rect.height, 40)}px`;
...
function createWordElement(word, isInDropzone, index = null) {
  ...
  if (isInDropzone) {
    ...
    el.addEventListener("dragend", (e) => {
      ...
      if (
        e.clientX < dropzoneRect.left || e.clientX > dropzoneRect.right ||
  ...
  el.addEventListener("mousedown", (e) => {
    ...
    function onMouseMove(e) {
      if (!dragged && (Math.abs(e.clientX - startX) > 5 || Math.abs(e.clientY - startY) > 5)) dragged = true;
    ...
    function onMouseUp(e) {
      ...
      if (inDropzone && ![...dropzone.children].some(child => child.textContent === word)) {
  ...
  el.addEventListener("click", () => {
    ...
    const isFromDropzone = isInDropzone;
    const rect = el.getBoundingClientRect();
    const scrollX = window.scrollX || window.pageXOffset;
    const scrollY = window.scrollY || window.pageYOffset;
    const startX = rect.left + scrollX;
    const startY = rect.top + scrollY;
    const index = isFromDropzone ? el.dataset.originIndex : el.dataset.index;

    const slot = wordChoices.querySelector(`.word-slot[data-index="${index}"]`);
    if (!isFromDropzone && slot) {
      slot.innerHTML = "";
      slot.appendChild(createPlaceholder(rect));
    }
    ...
    requestAnimationFrame(() => {
      ...
      const targetX = targetRect.left + 10 + dropzone.children.length * (rect.width + 10) + scrollX;
