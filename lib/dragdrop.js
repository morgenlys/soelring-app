...
function createWordElement(word, isInDropzone, index = null) {
  ...
  el.addEventListener("click", () => {
    ...
    // Entferne Original nach dem ersten Frame (damit Animation korrekt startet)
    requestAnimationFrame(() => {
      el.remove();

      // Zielkoordinaten berechnen
      const lastDropChild = [...dropzone.children].pop();
      const targetRect = lastDropChild
        ? lastDropChild.getBoundingClientRect()
        : dropzone.getBoundingClientRect();

      const targetX = targetRect.left + scrollX + (lastDropChild ? lastDropChild.offsetWidth + 10 : 10);
