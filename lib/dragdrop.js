...
function createWordElement(word, isInDropzone, index = null) {
  ...
  el.addEventListener("click", () => {
    ...
    // Aktuelle Position des Elements speichern
    const originalRect = el.getBoundingClientRect();
    const scrollX = window.scrollX || window.pageXOffset;
    const scrollY = window.scrollY || window.pageYOffset;
    const startX = originalRect.left + scrollX;
    const startY = originalRect.top + scrollY;
    ...
    if (!isInDropzone && slot) {
      slot.innerHTML = "";
      slot.appendChild(createPlaceholder(originalRect));
    }
    ...
    requestAnimationFrame(() => {
      ...
      const targetX = targetRect.left + 10 + target.children.length * (originalRect.width + 10) + scrollX;
