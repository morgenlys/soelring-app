...
function createPlaceholderMatching(el) {
...
function createWordElement(word, isInDropzone, index = null) {
      ...
      if (
        ...
        if (slot) {
          ...
          setTimeout(() => {
            ...
            if (dropzone.querySelectorAll(".word").length === 0) disableCheckButton();
          }, 300);
        }
      }
    });
  ...
  el.addEventListener("mousedown", (e) => {
    ...
    function onMouseUp(e) {
      ...
      if (inDropzone && !Array.from(dropzone.children).some(child => child.textContent === word)) {
        ...
        const placeholder = createPlaceholderMatching(el);
  ...
  el.addEventListener("click", () => {
    ...
    const isInDropzone = dropzone.contains(el);
    ...
    if (isInDropzone) {
      ...
      const targetX = endRect.left + 10 + dropzone.children.length * 80;
